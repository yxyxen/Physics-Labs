...
#        return a + 11605*t*x
    def func3(x,a):
        return a+np.log(np.exp( 11605*t*x) -1)
#find a fit that has smallest relative error on the y interception    
    for x in range(p1,p2):
        for y in range(x+p3,p2):
            ...
            x0    = np.array([-10]) # Initial guess for a and b, the parameters of the fit
    ...
    r =  total[(np.argmin(total, axis = 0))[2]]    #[c,a,da,b,db,x,y] with smallest c   
    ...
    pl.plot([x1, x2], [func3(x1,a), func3(x2,a)]) 
