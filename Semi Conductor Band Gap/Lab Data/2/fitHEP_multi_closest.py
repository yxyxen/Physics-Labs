...
for f in fileList:
    ...
    for x in range(p1,p2):
        for y in range(x+p3,p2):
            ...
            filetotal.append([a,da,b,db,x,y])
...
for f in fileList:
    orecarray = read_array(f, mydescr)
    pl.figure(1)
    pl.errorbar(orecarray.xpos[r[5][i][4]:r[5][i][5]],orecarray.ypos[r[5][i][4]:r[5][i][5]],orecarray.yerr[r[5][i][4]:r[5][i][5]],orecarray.xerr[r[5][i][4]:r[5][i][5]], fmt = 'o')
    i+=1
