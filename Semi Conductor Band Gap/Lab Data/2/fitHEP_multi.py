...
def read_array(filename, dtype, separator=','):
    ...
    for i in range(0,4):
        if filename == ('Ge_boiling.csv'):
            data[i] = data[i][0:-5]
        elif filename == ('Ge_room.csv'):
            data[i] = data[i][0:-5]
        elif filename == ('Ge_ice.csv'):
            data[i] = data[i][0:-5]
        elif filename == ('Ge_dryice.csv'):
            data[i] = data[i][0:-5]
#    print("data array is ")
#    print(data)
...
fileList = ['Ge_boiling.csv', 'Ge_room.csv', 'Ge_ice.csv', 'Ge_dryice.csv']
for f in fileList:
    myrecarray = read_array(f, mydescr)
...
#plot the least square fitted line.
    a = result[0][0]
    b = result[0][1]
    x1 = (-np.amax(myrecarray.xpos) + 11*np.amin(myrecarray.xpos))*0.1
    x2 = (11*np.amax(myrecarray.xpos) - np.amin(myrecarray.xpos))*0.1
